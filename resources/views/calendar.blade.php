        ...
        <div class="flex flex-col lg:flex-row flex-grow overflow-hidden">
            ...
            <div class="w-full lg:w-1/3 xl:w-1/4 bg-white p-6 order-1 lg:order-2 flex flex-col overflow-y-auto">
                ...
                <div class="mt-auto pt-6 flex-shrink-0">
                    <a href="{{ route('calendar.export', ['year' => $year, 'month' => $month]) }}" class="flex w-full items-center justify-center bg-blue-600 text-white px-4 py-3 rounded-lg hover:bg-blue-700 transition-colors duration-300 font-medium">
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col h-full w-full">
                    <div class="p-3 border-b border-gray-200">
                        <div class="flex flex-col sm:flex-row items-center justify-between">
                            <h1 class="text-2xl ms-4 font-bold text-gray-900">{{ \Carbon\Carbon::create($year, $month)->isoFormat('MMMM YYYY') }}</h1>
                            <div class="flex items-center space-x-2 mt-4 sm:mt-0">
                                <a href="{{ route('calendar.index', ['year' => $prevYear, 'month' => $prevMonth]) }}" class="p-2 rounded-full text-gray-500 hover:bg-gray-100"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg></a>
                                <a href="{{ route('calendar.index', ['year' => $nextYear, 'month' => $nextMonth]) }}" class="p-2 rounded-full text-gray-500 hover:bg-gray-100"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg></a>
                            </div>
                        </div>
                    </div>
                            ...
                            @foreach ($week as $day)
                                @if ($day)
                                    ...
                                    <div class="relative bg-white p-2 flex flex-col {{ $hasData ? 'cursor-pointer hover:scale-105 hover:shadow-lg hover:z-10 data-day' : '' }}"
                                        ...
                                        @if ($hasData)
                                            <div class="mt-1.5 flex-grow"><ul class="text-xs space-y-1 pr-1">
                                                <li class="flex items-center text-green-600"><span class="w-2 h-2 bg-green-500 rounded-full mr-2"></span><span class="truncate">GR: <strong>{{ number_format($data[$dateKey]['gr'], 0, ',', '.') }}</strong></span></li>
                                                <li class="flex items-center text-violet-600"><span class="w-2 h-2 bg-violet-500 rounded-full mr-2"></span><span class="truncate">Value: <strong>${{ number_format($data[$dateKey]['Total Value'], 0, ',', '.') }}</strong></span></li>
                                            </ul></div>
                                        @endif
                                    </div>
                                @else
